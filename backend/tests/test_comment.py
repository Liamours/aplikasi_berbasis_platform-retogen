...
def test_add_comment_single_char_content(client, auth_headers_user):
...
def test_add_comment_content_max_length_8192(client, auth_headers_user):
...
def test_add_comment_content_over_max(client, auth_headers_user):
...
def test_add_comment_admin_can_comment(client, auth_headers_admin):
...
def test_add_comment_returns_comment_list(client, auth_headers_user):
...
def test_add_reply_to_reply(client, auth_headers_user):
...
def test_edit_comment_success(client, auth_headers_user):
...
def test_edit_comment_empty_content(client, auth_headers_user):
...
def test_edit_comment_over_max_length(client, auth_headers_user):
...
def test_edit_comment_no_token(client):
...
def test_edit_comment_invalid_id(client, auth_headers_user):
...
def test_edit_comment_not_owner(client, auth_headers_admin):
    print("\n[TEST CASE] Edit Comment - Bukan Pemilik (Admin Trying to Edit User Comment)")
    response = client.post("/comment/edit/update", json={
        "comment_id": COMMENT_ID_SEED,
        "article_id": ARTICLE_ID,
...
def test_edit_get_comment_owner_success(client, auth_headers_user):
...
def test_edit_get_comment_no_token(client):
    print("\n[TEST CASE] Edit Get Comment - Tanpa Token")
    response = client.post("/comment/edit/get", json={"comment_id": COMMENT_ID_SEED})
    assert response.status_code == 401


# ── new tests: delete comment ─────────────────────────────────────────────────

def test_delete_comment_no_token(client):
    print("\n[TEST CASE] Delete Comment - Tanpa Token")
    response = client.post("/comment/delete", json={"comment_id": COMMENT_ID_SEED})
    assert response.status_code == 401
...
def test_delete_comment_not_owner_regular_user(client, auth_headers_user, auth_headers_admin):
...
def test_delete_comment_owner_can_delete_own(client, auth_headers_user):
